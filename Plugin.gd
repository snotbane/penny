...
static func init_project_setting(hint: Dictionary, default_value: Variant) -> void:
	assert(hint.has(&"name"), "Project setting missing name.")
	assert(hint.has(&"type"), "Project setting missing type.")

	## Doing it this way wnsures the correct order.
	var temp = ProjectSettings.get_setting(hint[&"name"], default_value)
	ProjectSettings.set_setting(hint[&"name"], null)
	ProjectSettings.set_setting(hint[&"name"], temp)

	ProjectSettings.add_property_info(hint)
	ProjectSettings.set_initial_value(hint[&"name"], default_value)

func _enter_tree() -> void:

	init_project_setting(Penny.PSH_START_LABEL, Penny.PSH_START_LABEL_DEFAULT)
	init_project_setting(Penny.PSH_ALLOW_ROLLING, Penny.PSH_ALLOW_ROLLING_DEFAULT)
	init_project_setting(Penny.PSH_ALLOW_SKIPPING, Penny.PSH_ALLOW_SKIPPING_DEFAULT)
	init_project_setting(DecorRegistry.PROJECT_SETTING_HINT, DecorRegistry.PROJECT_SETTING_DEFAULT_VALUE)
